...
#emmax_log2/log2.OTU_12.sig.ps   1       36655774        36659693        3919    1m36655774      3.195e-08       2
#emmax_log2/log2.OTU_12.sig.ps   3       27929713        27929713        0       3m27929713      6.0614e-07      1
#emmax_log2/log2.OTU_12.sig.ps   8       331953  331953  0       8m331953        8.1162e-07      1
#emmax_log2/log2.OTU_12.sig.ps   8       648978  648978  0       8m648978        8.4291e-08      1
...
while(<INPUT>){
	...
	if($chr eq $CHR){
		if ($pos - $P1 <= $opts{d}){
			...
			$range = $end - $start;
			print OUTPUT "$opts{i}\t$CHR\t$start\t$end\t$range\t$ID1\t$PV1\t$n\n";
			$P1 = $pos;
			$n = 1;
			$start = $pos;
			$end = $pos;
