...
if (TRUE){
  option_list <- list(
    make_option(c("-g", "--genotype"), type="character", default="../snpTtest/4m27567935.ped",
...
main_theme = theme(panel.background=element_blank(), panel.grid=element_blank(),
                   ...
                   legend.text= element_text(size=7),text=element_text(family="sans", size=7),
...
#  stat_compare_means(method = opts$method)+xlab(gsub(".ped","",basename(opts$genotype)))+ylab(basename(opts$phenotype))+main_theme+
#  theme(legend.position = "NA")
#ggsave(paste0(opts$output,opts$method,".pdf"), p, width = opts$width, height = opts$height, units = "mm")
#ggsave(paste0(opts$output,opts$method,".png"), p, width = opts$width, height = opts$height, units = "mm")
# p
