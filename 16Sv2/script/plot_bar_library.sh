...
cat <<END >script/plot_bar_library.r
...
sample_count = read.table("${input}", header=F, sep="\t") # dataframe
...
#p
ggsave("${output}.pdf", p, width = ${width}, height = ${height})
ggsave("${output}.png", p, width = ${width}, height = ${height})
...
then
	Rscript script/plot_bar_library.r
fi
